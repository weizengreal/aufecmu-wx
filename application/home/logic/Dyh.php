...
class Dyh{
    ...
    public function userGetCard($openid ,$code ,$cardid) {
        ...
        if(! empty($userInfo['subscribe']) && $userInfo['subscribe'] != "0" ) {
            // 用户已经关注
            $vipArr += ['subscribe'=>1];
        }
        else {
        ...
        if($this->vipuser->isExist(array('openid'=>"$openid"))) {
            unset($vipArr['openid']);
            return $this->updateInfo($openid,$vipArr);
        }
        else {
            return $this->vipuser->addNew($vipArr);
        }
    }

    public function userViewCard($openid,$code,$cardId) {
        return $this->vipuser->updata($openid,array(
            'code'=>$code,
            'cardid'=>"$cardId",
        )) !== false;
    }
